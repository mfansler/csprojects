...
(defun estimated-cost (p0 p1 ps) (
  + (get-distance p0 p1)
    (log (reduce #'(lambda (total q) (+ total (get-distance p1 q))) ps :initial-value 1))
))

(defun least-cost (p0 ps) #'(lambda (p1 p2) (< (estimated-cost p0 p1 ps) (estimated-cost p0 p2 ps))))


(let* ((y (read)) (x (read)) (w (read)) (h (read)) (s (read-state 0 (list (list x y) ())))) 
